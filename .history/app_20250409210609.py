...
def parse_cisco_interface_details(details):
    """Парсинг деталей интерфейса для Cisco устройств"""
    return {
        'description': re.search(r'Description: (.+)', details).group(1).strip() if 'Description:' in details else 'N/A',
        ...
        'speed': re.search(r'BW (\d+)', details).group(1) + ' Kbps' if 'BW' in details else 'N/A',
        'duplex': re.search(r'Duplex:(.+?),', details).group(1).strip().lower() if 'Duplex:' in details else 'N/A',
        'mtu': re.search(r'MTU (\d+)', details).group(1) if 'MTU' in details else '1500',
        'mac_address': re.search(r'address is (.+?) ', details).group(1) if 'address is' in details else 'N/A',
        'ip_address': re.search(r'Internet address is (.+?)[,\s]', details).group(1) if 'Internet address is' in details else 'N/A',
        'netmask': re.search(r'Internet address is .+?/(\d+)', details).group(1) if 'Internet address is' in details else 'N/A',
