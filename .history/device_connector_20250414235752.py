...
def parse_eltex_cpu_memory(output):
    ...
    try:
        ...
        # Парсим CPU
        cpu_section = output.split('CPU utilization')
        if len(cpu_section) > 1:
            cpu_lines = cpu_section[1].split('------------')
            if len(cpu_lines) > 1:
                for line in cpu_lines[1].splitlines():
