    ...
    for line in interfaces_str.splitlines()[1:]:  # Пропускаем заголовок
        if line.strip():
            ...
            if len(parts) >= 6:
                ...
                interface_ip = parts[1] if parts[1] != 'unassigned' else 'N/A'
                
                # Для Cisco: если IP в формате x.x.x.x/y, разделяем на IP и маску
                ip_parts = interface_ip.split('/') if interface_ip != 'N/A' else []
                ip_address = ip_parts[0] if ip_parts else 'N/A'
