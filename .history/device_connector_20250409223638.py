...
def create_interface_on_device(device_data, interface_data):
    ...
    try:
        ...
        app.logger.info(f"Подключение для создания интерфейса {interface_data['name']}")
        connection = ConnectHandler(**device_params)
        ...
        try:
            ...
            app.logger.debug(f"Отправка команд для создания интерфейса: {commands}")
            output = connection.send_config_set(commands)
            app.logger.debug(f"Результат выполнения команд:\n{output}")
            ...
            if device_type == 'cisco':
                ...
                app.logger.debug(f"Результат сохранения конфигурации:\n{save_output}")
            else:  # Huawei
            ...
            app.logger.error(f"Ошибка при создании интерфейса: {str(e)}")
            return False, str(e)
        finally:
            if connection:
                connection.disconnect()
        ...
        app.logger.error(f"Ошибка подключения для создания интерфейса: {str(e)}")
        return False, str(e)