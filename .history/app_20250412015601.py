...
def update_interface():
    ...
    try:
        ...
        success, result = update_interface_on_device(device_data, data)
        ...
        if success:
            ...
            return jsonify({
                'success': True,
                'message': result.get('message', 'Интерфейс успешно обновлен')
            })
        else:
            return jsonify({
                'success': False,
                'message': result.get('message', 'Не удалось обновить интерфейс'),
                'suggestions': result.get('suggestions', [])
            }), 400
        
    except Exception as e:
        app.logger.error(f"Ошибка в update_interface: {str(e)}")
        return jsonify({
            'success': False,
