...
def update_vlan():
    ...
    try:
        ...
        # Здесь реализуйте логику обновления VLAN на устройстве
        # Пример для Cisco:
        connection = ConnectHandler(
        ...
        try:
            ...
            if data.get('description'):
                commands.append(f"description {data['description']}")
            
            # Для режима порта потребуется дополнительная логика
            # в зависимости от конкретного интерфейса
