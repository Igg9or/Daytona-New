...
def update_interface_on_device(device_data, interface_data):
    ...
    try:
        ...
        logger.info(f"Подключение для обновления интерфейса {interface_data['interface_name']}")
        connection = ConnectHandler(**device_params)
        ...
        if device_data.get('secret'):
            try:
                ...
                logger.error(f"Ошибка входа в enable режим: {str(enable_error)}")
                return False, {
                    'message': 'Не удалось войти в привилегированный режим',
                    'suggestions': [
                        'Проверьте правильность enable пароля',
                        'Убедитесь, что у пользователя есть права на конфигурацию'
                    ]
                }
        ...
        logger.debug(f"Отправка команд: {commands}")
        ...
        try:
            ...
            for cmd in commands:
                ...
                if 'Invalid input' in output or 'Error' in output:
                    ...
                    suggestions = [
                        ...
                        'Проверьте синтаксис команды для данного устройства'
                    ]
                    if 'Invalid vlan' in output:
                        error_msg = "Указан несуществующий VLAN"
                        suggestions.append('Сначала создайте VLAN или укажите существующий')
            ...
            if device_type == 'cisco':
                save_output = connection.send_command('write memory', delay_factor=2)
                logger.debug(f"Сохранение конфигурации: {save_output[:200]}...")
            
            return True, {'message': 'Интерфейс успешно обновлен'}
            
        except Exception as cmd_error:
            logger.error(f"Ошибка выполнения команд: {str(cmd_error)}")
            error_info = {
                'message': 'Ошибка при настройке интерфейса',
                'suggestions': [
            ...
            if isinstance(cmd_error, dict):
                error_info.update(cmd_error)
            return False, error_info
            
    except NetmikoAuthenticationException as auth_error:
        logger.error(f"Ошибка аутентификации: {str(auth_error)}")
        return False, {
            'message': 'Ошибка аутентификации',
            'suggestions': [
                'Проверьте логин и пароль',
                'Убедитесь, что устройство поддерживает SSH',
                'Проверьте enable пароль (если требуется)'
            ]
        }
        ...
        if connection:
            try:
                ...
                logger.error(f"Ошибка при закрытии соединения: {str(disconnect_error)}")
