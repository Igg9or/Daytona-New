...
def get_real_vlan_details(device_data, vlan_id):
    """Получаем детальную информацию о VLAN с улучшенной обработкой ошибок"""
    connection = None
    try:
        device_type = device_data.get('device_type', 'Cisco').lower()
        netmiko_device_type = 'cisco_ios' if device_type == 'cisco' else 'huawei'
        
        # Конфигурация подключения с увеличенными таймаутами
        device_params = {
            'device_type': netmiko_device_type,
            'host': device_data['ip_address'],
            'username': device_data['username'],
            ...
            'global_delay_factor': 2,
            'banner_timeout': 20,
            'session_log': 'vlan_details_session.log'
        }

        app.logger.info(f"Подключение для получения деталей VLAN {vlan_id}")
        connection = ConnectHandler(**device_params)
        
        # Принудительно включаем режим enable
        try:
            if device_data.get('secret'):
                connection.enable()
        except Exception as enable_error:
            app.logger.error(f"Ошибка enable режима: {str(enable_error)}")
            raise Exception(f"Не удалось войти в enable режим: {str(enable_error)}")
        ...
        if device_type == 'cisco':
            commands = [
                ...
                f'show ip interface brief | include Vlan{vlan_id}'
            ]
        # Для Huawei устройств
        else:
            commands = [
                f'display vlan {vlan_id}',
                f'display interface Vlanif{vlan_id}',
        ...
        for cmd in commands:
            try:
                app.logger.debug(f"Отправка команды: {cmd}")
                ...
                outputs[cmd] = output
                app.logger.debug(f"Результат ({cmd}): {output[:200]}...")  # Логируем первые 200 символов
            except Exception as cmd_error:
                app.logger.error(f"Ошибка выполнения команды {cmd}: {str(cmd_error)}")
                outputs[cmd] = f"ERROR: {str(cmd_error)}"

        # Парсим выходные данные
        if device_type == 'cisco':
            vlan_details = parse_cisco_vlan_details(vlan_id, outputs)
        else:
            vlan_details = parse_huawei_vlan_details(vlan_id, outputs)
        ...
        app.logger.error(f"Ошибка аутентификации: {str(auth_error)}")
        raise Exception("Ошибка аутентификации: неверные учетные данные")
    except NetmikoTimeoutException as timeout_error:
        app.logger.error(f"Таймаут подключения: {str(timeout_error)}")
        raise Exception("Устройство не отвечает - превышено время ожидания")
    except Exception as e:
        app.logger.error(f"Общая ошибка: {str(e)}", exc_info=True)
        ...
        if connection:
            try:
                ...
                app.logger.error(f"Ошибка при закрытии соединения: {str(disconnect_error)}")
...
def parse_cisco_vlan_details(vlan_id, outputs):
    """Парсинг данных VLAN для Cisco с защитой от ошибок"""
    details = {
        'name': '',
        'status': 'inactive',
        'description': '',
    ...
    try:
        ...
        if 'invalid' not in vlan_output.lower():
            for line in vlan_output.splitlines():
                if re.match(rf'^{vlan_id}\s+\w+', line):
                    parts = re.split(r'\s+', line.strip())
                    details['name'] = parts[1]
                    details['status'] = 'active' if parts[2].lower() == 'active' else 'inactive'
                    details['ports'] = [p for p in parts[3:] if p and p != '--']
                    break

        # Парсим show interface Vlan
        interface_output = outputs.get(f'show interface Vlan{vlan_id}', '')
        for line in interface_output.splitlines():
            if 'Internet address is' in line:
                match = re.search(r'Internet address is (.+?)[,\s]', line)
                if match:
                    details['ip'] = match.group(1).split('/')[0]
            elif 'MTU' in line:
                match = re.search(r'MTU (\d+)', line)
                if match:
                    details['mtu'] = match.group(1)
            elif 'Hardware is' in line and 'address is' in line:
                match = re.search(r'address is (.+?)\s', line)
                if match:
                    details['mac'] = match.group(1)

        # Парсим show run interface
        run_output = outputs.get(f'show run interface Vlan{vlan_id}', '')
        details['dhcp'] = 'Enabled' if 'ip helper-address' in run_output else 'Disabled'

    except Exception as e:
        app.logger.error(f"Ошибка парсинга данных Cisco VLAN: {str(e)}")

    return details

def parse_huawei_vlan_details(vlan_id, outputs):
    """Парсинг данных VLAN для Huawei с защитой от ошибок"""
    details = {
        'name': '',
        'status': 'inactive',
        'description': '',
    ...
    try:
        # Парсим display vlan
        vlan_output = outputs.get(f'display vlan {vlan_id}', '')
        for line in vlan_output.splitlines():
            if 'VLAN Name' in line:
                details['name'] = line.split(':')[1].strip()
            elif 'Description' in line:
                details['description'] = line.split(':')[1].strip()
            elif 'Tagged' in line or 'Untagged' in line:
                ports = line.split(':')[1].strip().split()
                details['ports'].extend(ports)

        # Парсим display interface Vlanif
        interface_output = outputs.get(f'display interface Vlanif{vlan_id}', '')
        for line in interface_output.splitlines():
            if 'Internet Address is' in line:
                match = re.search(r'Internet Address is (.+?)/', line)
                if match:
        ...
        # Парсим конфигурацию
        config_output = outputs.get(f'display current-configuration interface Vlanif{vlan_id}', '')
        details['dhcp'] = 'Enabled' if 'dhcp select' in config_output else 'Disabled'

    except Exception as e:
        app.logger.error(f"Ошибка парсинга данных Huawei VLAN: {str(e)}")
