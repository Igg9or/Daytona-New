...
def update_interface_on_device(device_data, interface_data):
    """Обновление интерфейса с поддержкой IP и маски"""
    connection = None
    try:
        device_type = device_data.get('device_type', 'Cisco').lower()
        netmiko_device_type = 'cisco_ios' if device_type == 'cisco' else 'huawei'
        ...
        commands = [f"interface {interface_data['interface_name']}"]
        ...
        # Настройка IP-адреса
        if interface_data.get('ip_address') and interface_data.get('netmask'):
            if device_type == 'huawei':
                commands.append(f"ip address {interface_data['ip_address']} {interface_data['netmask']}")
            else:
                commands.append(f"ip address {interface_data['ip_address']} {interface_data['netmask']}")
        
        # Настройка дуплекса и скорости (для физических интерфейсов)
        if not interface_data['interface_name'].lower().startswith(('vlan', 'loopback')):
            if 'duplex' in interface_data:
                commands.append(f"duplex {interface_data['duplex']}")
            if 'speed' in interface_data and interface_data['speed'] != 'auto':
                commands.append(f"speed {interface_data['speed']}")
        
        # Управление статусом интерфейса
        if interface_data.get('status') == 'up':
            commands.append("no shutdown" if device_type == 'cisco' else "undo shutdown")
        else:
            commands.append("shutdown" if device_type == 'cisco' else "shutdown")
        ...
        error_msg = f"Ошибка при обновлении интерфейса: {str(e)}"
