...
def collect_real_device_data(connection, device_data):
    ...
    if device_type == 'eltex':
        ...
        system_output = connection.send_command('show system')
        system_data = parse_eltex_system(system_output)
        
        # Получаем информацию о CPU и памяти
        cpu_output = connection.send_command('show cpu utilization')
        cpu_memory_data = parse_eltex_cpu_memory(cpu_output)
        
        # Получаем температуру
        temp_output = connection.send_command('show system sensors')
        ...
        return {
            ...
            'configuration': {
                ...
                'gateway': 'N/A',  # Для Eltex нужно отдельно получать шлюз
...
# Вспомогательные функции для Eltex
...
def parse_eltex_cpu_memory(output):
    ...
    for line in output.splitlines():
        if 'Total/Free:' in line:
            parts = line.split('Total/Free:')[1].split()
            total = parts[0]
            free = parts[1]
            percent = line.split('(')[1].split('%')[0] if '(' in line else '0'
            data['memory_usage'] = f"{free}/{total} ({percent}%)"
    
    # Парсим CPU
    cpu_lines = output.split('CPU utilization')[1].split('------------')[1].splitlines()
    for line in cpu_lines:
        if 'five seconds:' in line:
            cpu_load = line.split(':')[1].split('%')[0].strip()
...
def parse_eltex_temperature(output):
    ...
    data = {}
    # Ищем первую температуру (основную)
    for line in output.splitlines():
